...
export const getUsaStateList = () => {
  ...
  const stateList = usaCountryData.map((elem: any) => ({
    label: `${elem.name}, ${elem.isoCode}`,
    value: elem.isoCode,
  }));
...
export const getUsaCityList = (state: string) => {
  const selectedState = usaCountryData?.find((el: any) => el.isoCode === state);
  ...
  return (
    selectedState?.cities?.map((elem: any) => ({
      label: elem.name,
      value: elem.name,
    })) || []
  );
