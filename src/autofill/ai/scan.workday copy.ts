...
export const collectWorkdayCandidateFields = (): WorkdayCandidateField[] => {
  ...
  document
    ...
    .forEach((checkbox) => {
      if (isInsideExtension(checkbox)) return;
      const isCurrentWorkCheckbox =
        checkbox.name === "currentlyWorkHere" ||
        checkbox.id?.toLowerCase().includes("currentlyworkhere");
      // Native Workday checkbox is often opacity-0 / zero-sized behind custom chrome
      if (!isCurrentWorkCheckbox && !isVisibleElement(checkbox)) return;
