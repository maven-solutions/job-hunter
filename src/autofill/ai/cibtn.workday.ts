...
import { collectWorkdayCandidateFields } from "./scan.workday";
import {
...
const SCAN_ICON_CLASS = "careerai-workday-scan-field-icon";
const SCAN_ICON_WRAPPER_CLASS = "careerai-workday-scan-icon-wrapper";
const SCAN_STYLE_ID = "careerai-workday-scan-html-styles";
...
const findIconAnchor = (element: HTMLElement): HTMLElement => {
  const entry = element.closest(
    '[data-automation-id^="formField-"], [data-fkit-id]',
  ) as HTMLElement | null;
...
const buildApiElementForField = (entry: ScannableFieldEntry): AiFormElement => ({
  ...
  required: entry.element.getAttribute("aria-required") === "true",
  type: "text",
});
...
const attachIconToField = (entry: ScannableFieldEntry): void => {
  ...
  button.addEventListener("click", async (event) => {
    ...
    try {
      ...
      console.error("[CareerAI FieldAI:workday]", error);
...
/**
 * Injects field icons on **textareas only** for the current Workday page.
 * Other fields are filled via full Autofill with AI.
 */
export const initWorkdayHtmlScanner = (
  _applicantData: ApplicantContext | null = null,
  options: AiFieldScannerOptions = {},
): number => {
  injectScanStyles();
  removeWorkdayHtmlScannerIcons();
  requestFieldAnswerFn = options.requestFieldAnswer ?? null;

  const textareaCandidates = collectWorkdayCandidateFields().filter(
...
export const removeWorkdayHtmlScannerIcons = (): void => {
  document
...
export const getWorkdayScannedFieldCount = (): number => scannedFields.size;
