...
import { collectJobviteCandidateFields } from "./scan.jobvite";
import {
...
const SCAN_ICON_CLASS = "careerai-jobvite-scan-field-icon";
const SCAN_ICON_WRAPPER_CLASS = "careerai-jobvite-scan-icon-wrapper";
const SCAN_STYLE_ID = "careerai-jobvite-scan-html-styles";
...
const findIconAnchor = (element: HTMLElement): HTMLElement => {
  const field = element.closest(".jv-form-field") as HTMLElement | null;
  if (field) return field;

  const control = element.closest(
    ".jv-form-field-control, .jv-form-field-textarea",
  ) as HTMLElement | null;
...
const attachIconToField = (entry: ScannableFieldEntry): void => {
  ...
  button.addEventListener("click", async (event) => {
    ...
    try {
      ...
      console.error("[CareerAI FieldAI:jobvite]", error);
...
/**
 * Scans the Jobvite application form and injects field icons on **textareas only**.
 * Inputs, selects, and currency fields are filled only via full Autofill with AI.
 */
export const initJobviteHtmlScanner = (
  _applicantData: ApplicantContext | null = null,
  options: AiFieldScannerOptions = {},
): number => {
  injectScanStyles();
  removeJobviteHtmlScannerIcons();
  requestFieldAnswerFn = options.requestFieldAnswer ?? null;

  const textareaCandidates = collectJobviteCandidateFields().filter(
    (
      candidate,
...
export const removeJobviteHtmlScannerIcons = (): void => {
  document
...
export const getJobviteScannedFieldCount = (): number => scannedFields.size;
