...
const SCAN_ICON_CLASS = "careerai-applytojob-scan-field-icon";
const SCAN_ICON_WRAPPER_CLASS = "careerai-applytojob-scan-icon-wrapper";
const SCAN_STYLE_ID = "careerai-applytojob-scan-html-styles";
...
const findIconAnchor = (element: HTMLElement): HTMLElement => {
  const group = element.closest(".form-group, .resumator-input") as HTMLElement | null;
...
const attachIconToField = (entry: ScannableFieldEntry): void => {
  ...
  button.addEventListener("click", async (event) => {
    ...
    try {
      ...
      console.error("[CareerAI FieldAI:applytojob]", error);
...
/**
 * Scans the JazzHR / ApplyToJob form and injects field icons on **textareas only**.
 * Inputs and native selects are filled only via full Autofill with AI.
 */
export const initApplyToJobHtmlScanner = (
  _applicantData: ApplicantContext | null = null,
  options: AiFieldScannerOptions = {},
): number => {
  injectScanStyles();
  removeApplyToJobHtmlScannerIcons();
  requestFieldAnswerFn = options.requestFieldAnswer ?? null;

  const textareaCandidates = collectApplyToJobCandidateFields().filter(
    (
      candidate,
...
export const removeApplyToJobHtmlScannerIcons = (): void => {
  document
...
export const getApplyToJobScannedFieldCount = (): number => scannedFields.size;
