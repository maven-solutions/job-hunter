...
import { collectUltiproCandidateFields } from "./scan.ultipro";
import {
...
const SCAN_ICON_CLASS = "careerai-ultipro-scan-field-icon";
const SCAN_ICON_WRAPPER_CLASS = "careerai-ultipro-scan-icon-wrapper";
const SCAN_STYLE_ID = "careerai-ultipro-scan-html-styles";
...
const findIconAnchor = (element: HTMLElement): HTMLElement => {
  const group = element.closest(".form-group") as HTMLElement | null;
...
const attachIconToField = (entry: ScannableFieldEntry): void => {
  ...
  button.addEventListener("click", async (event) => {
    ...
    try {
      ...
      console.error("[CareerAI FieldAI:ultipro]", error);
...
/**
 * Scans the UKG / Ultipro form and injects field icons on **textareas only**.
 * Contact Information has none; later sections with textareas get icons.
 */
export const initUltiproHtmlScanner = (
  _applicantData: ApplicantContext | null = null,
  options: AiFieldScannerOptions = {},
): number => {
  injectScanStyles();
  removeUltiproHtmlScannerIcons();
  requestFieldAnswerFn = options.requestFieldAnswer ?? null;

  const textareaCandidates = collectUltiproCandidateFields().filter(
    (
      candidate,
...
export const removeUltiproHtmlScannerIcons = (): void => {
  document
...
export const getUltiproScannedFieldCount = (): number => scannedFields.size;
