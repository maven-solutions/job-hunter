...
import { collectBambooHrCandidateFields } from "./scan.bamboohr";
import {
...
const SCAN_ICON_CLASS = "careerai-bamboohr-scan-field-icon";
const SCAN_ICON_WRAPPER_CLASS = "careerai-bamboohr-scan-icon-wrapper";
const SCAN_STYLE_ID = "careerai-bamboohr-scan-html-styles";
...
const findIconAnchor = (element: HTMLElement): HTMLElement => {
  const wrapper = element.closest(
    '[data-fabric-component="TextArea InputWrapper"], .MuiFormControl-root, .CandidateField',
  ) as HTMLElement | null;
...
const attachIconToField = (entry: ScannableFieldEntry): void => {
  ...
  button.addEventListener("click", async (event) => {
    ...
    try {
      ...
      console.error("[CareerAI FieldAI:bamboohr]", error);
...
/**
 * Scans the BambooHR application form and injects field icons on **textareas only**.
 * Inputs, Fabric selects, and radio groups are filled only via Autofill with AI.
 */
export const initBambooHrHtmlScanner = (
  _applicantData: ApplicantContext | null = null,
  options: AiFieldScannerOptions = {},
): number => {
  injectScanStyles();
  removeBambooHrHtmlScannerIcons();
  requestFieldAnswerFn = options.requestFieldAnswer ?? null;

  const textareaCandidates = collectBambooHrCandidateFields().filter(
    (
      candidate,
...
export const removeBambooHrHtmlScannerIcons = (): void => {
  document
...
export const getBambooHrScannedFieldCount = (): number => scannedFields.size;
