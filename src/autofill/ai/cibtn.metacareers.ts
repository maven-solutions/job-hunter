...
import { collectMetacareersCandidateFields } from "./scan.metacareers";
import {
...
const SCAN_ICON_CLASS = "careerai-metacareers-scan-field-icon";
const SCAN_ICON_WRAPPER_CLASS = "careerai-metacareers-scan-icon-wrapper";
const SCAN_STYLE_ID = "careerai-metacareers-scan-html-styles";

/** Per-field icons are textarea-only on Meta Careers. */
...
const attachIconToField = (entry: ScannableFieldEntry): void => {
  ...
  button.addEventListener("click", async (event) => {
    ...
    try {
      ...
      console.error("[CareerAI FieldAI:metacareers]", error);
...
/**
 * Scans the Meta Careers application form and injects field icons on
 * **textareas only**. Inputs, comboboxes, checkboxes, and radios are filled
 * only via the full Autofill with AI flow.
 */
export const initMetacareersHtmlScanner = (
  _applicantData: ApplicantContext | null = null,
  options: AiFieldScannerOptions = {},
): number => {
  injectScanStyles();
  removeMetacareersHtmlScannerIcons();
  requestFieldAnswerFn = options.requestFieldAnswer ?? null;

  const textareaCandidates = collectMetacareersCandidateFields().filter(
    (
      candidate,
...
export const removeMetacareersHtmlScannerIcons = (): void => {
  document
...
export const getMetacareersScannedFieldCount = (): number => scannedFields.size;
