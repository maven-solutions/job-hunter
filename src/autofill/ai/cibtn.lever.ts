...
import { collectLeverCandidateFields } from "./scan.lever";
import {
...
const SCAN_ICON_CLASS = "careerai-lever-scan-field-icon";
const SCAN_ICON_WRAPPER_CLASS = "careerai-lever-scan-icon-wrapper";
const SCAN_STYLE_ID = "careerai-lever-scan-html-styles";
...
const findIconAnchor = (element: HTMLElement): HTMLElement => {
  ...
  const field = element.closest(".application-field") as HTMLElement | null;
...
const attachIconToField = (entry: ScannableFieldEntry): void => {
  ...
  button.addEventListener("click", async (event) => {
    ...
    try {
      ...
      console.error("[CareerAI FieldAI:lever]", error);
...
/**
 * Scans the Lever application form and injects field icons on **textareas only**.
 * Inputs, selects, and checkbox groups are filled only via full Autofill with AI.
 */
export const initLeverHtmlScanner = (
  _applicantData: ApplicantContext | null = null,
  options: AiFieldScannerOptions = {},
): number => {
  injectScanStyles();
  removeLeverHtmlScannerIcons();
  requestFieldAnswerFn = options.requestFieldAnswer ?? null;

  const textareaCandidates = collectLeverCandidateFields().filter(
    (
      candidate,
...
export const removeLeverHtmlScannerIcons = (): void => {
  document
...
export const getLeverScannedFieldCount = (): number => scannedFields.size;
